...
let nodeBehavior (nodeID: int) m maxNumRequests convergenceCounterRef (mailbox: Actor<_>) =
    ...
    let rec loop () = actor {
        ...
        | FindNodeSuccessor (id) ->
            if(inBetweenWithoutLeftWithRight hashSpace nodeID id fingerTable.[0]) then
                let newNodePath = constructActorPath id
                ...
                while(i >= 0) do
                    if(inBetweenWithoutLeftWithoutRight hashSpace nodeID fingerTable.[i] id) then
                        ...
                        let closestPrecedingNodePath = constructActorPath closestPrecedingNodeID
        ...
        | Stabilize ->
            ...
            let succesorPath = constructActorPath successorID
        ...
        | FindFingerSuccessor (originNodeID, next, id) ->
            if(inBetweenWithoutLeftWithRight hashSpace nodeID id fingerTable.[0]) then
                let originNodePath = constructActorPath originNodeID
        ...
        | StartQuerying ->
            if(numRequests < maxNumRequests) then
                ...
                convergenceCounterRef <! InrementConvergedNode (nodeID, totalHopCount, numRequests)
...
let main argv =
    ...
    while(i < numNodes) do
        try
            ...
            nodeIDs.[i] <- nodeID
            nodeRefs.[i] <- spawn system (string nodeID) (nodeBehavior nodeID m numRequests convergenceCounterRef)
            if(i = 0) then
                nodeRefs.[i] <! Create
            else
